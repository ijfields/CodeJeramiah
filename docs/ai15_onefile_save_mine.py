...
# 1/9/24 - modified to work with my env variables
...
client2 = {
    ...
    # "groq": Groq(api_key=api_keys["groq"]),
    # "local": OpenAI(base_url="http://localhost:1234/v1", api_key="not-needed"),
}
def openai_version(user_input, system_message):
    ...
    chat_completion = client2["openai"].chat.completions.create(
        model="gpt-3.5-turbo-0125",  # Adjust the model as necessary
        messages=messages,
        temperature=0.6,
    )
...
def chat_gpt3(userinput, chatbot, temperature=0.7, frequency_penalty=0, presence_penalty=0, api_key='your-api-key'):
    ...
    response = openai.chat.completions.create(
        model="gpt-3.5-turbo",  # or "text-davinci-003" for the latest GPT-3 model
...
#     completion = openai_version(f"Suggest a potential filename for a trading strategy based on this description: {strategy_description}. The filename should conform to Windows standards and not include a file extension. Your response should only be one string.", "You are an experienced software engineer.")
#     #ai_suggested = completion.strip()
...
def generate_filename(strategy_description, extension):
    ...
    # Remove double quotes from the strategy name
...
def create_and_run_data_analysis(trading_idea):
    ...
    if data_analysis_output:
        ...
        #save_output_to_file(data_analysis_output, data_analysis_output, '/Users/tc/Dropbox/dev/github/Open-AI-Assistants/strategies_event', 'txt')
...
def create_and_run_backtest(strategy_output, trading_idea, filename_base):
    ...
    if backtest_output:
        ...
        #save_output_to_file(backtest_output, strategy_output, '/Users/tc/Dropbox/dev/github/Open-AI-Assistants/bt_code_event', 'py')
...
def classify_and_process_idea(idea):
    ...
    if youtube_match:
        ...
        if idea.startswith("http"):
            ...
            # Check if the idea is a local file path
...
def chunk_it(idea) : 
#all_text = open_file('youtubetext.txt')
    all_text = idea
    chunks = textwrap.wrap(all_text, 2500)
    result = []
    for i, chunk in enumerate(chunks, start=1):
        prompt = open_file('prompt.txt').replace('<<SUMMARY>>', chunk).encode('ascii', 'ignore').decode()
       
        summary = chat_gpt3(prompt,'you are a helpfull assistent knowlegable in python programming')
       
        print(f'{i} out of {len(chunks)} Compressions: {summary}')
